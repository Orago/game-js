...
export class WglProgram {
	...
	static fpi =
		OSF.destination.size +
		OSF.source.size +
		OSF.color.size +
		OSF.rotation.size;
	canvas = document.createElement("canvas");
	gl: WebGL2RenderingContext = this.canvas.getContext("webgl2")!;
...
export class TextureAtlas {
	...
	constructor(
		...
		img: TexImageSource & { width: number; height: number }
	) {
...
export class InstanceBuffer {
	...
	constructor(
		...
		public floats_per_instance: number,
	...
	ensureCapacity(min: number) {
		...
		const new_data = new Float32Array(
			Math.max(new_capacity * this.floats_per_instance, this.data.length)
		);
	...
	shake() {
		...
		const ordered: [number, TInstancedSpriteSlice][] = [];
	...
	getSelection(index: number): TInstancedSpriteSlice {
		const offset = index * this.floats_per_instance;
		const list: TInstancedSpriteSlice = InstanceBuffer.initial();
	...
	updateSelection(
		index: number,
		data: TSpriteOptions & { selection?: TInstancedSpriteSlice }
	) {
		...
		if (data.source != undefined) {
			selection.splice(
				OSF.source.start,
				OSF.source.size,
				...data.source.slice(0, OSF.source.size)
			);
		}
		if (data.destination != undefined) {
			selection.splice(
				OSF.destination.start,
				OSF.destination.size,
				...data.destination.slice(0, OSF.destination.size)
			);
		}
		...
		if (data.color != undefined) {
			selection.splice(
				OSF.color.start,
				OSF.color.size,
				...data.color.slice(0, OSF.color.size)
			);
		}
	...
	addInstance(
		array: TInstancedSpriteSlice = InstanceBuffer.initial()
	): number {
	...
	getValue(
		index: number,
		data: "source" | "destination" | "color" | "rotation"
	): any {
		...
		switch (data) {
			case "source":
				return selection.slice(
					OSF.source.start,
					OSF.source.start + OSF.source.size
				);
			case "destination":
				return selection.slice(
					OSF.destination.start,
					OSF.destination.start + OSF.destination.size
				);
			case "rotation":
			...
			case "color":
				return selection.slice(
					OSF.color.start,
					OSF.color.start + OSF.color.size
				);
		}
	}
	...
	upload() {
		...
		this.gl.bufferSubData(
			this.gl.ARRAY_BUFFER,
			0,
			this.data.subarray(0, this.count * this.floats_per_instance)
		);
	}

	createSprite(options?: TSpriteOptions) {
...
export class WGLSprite {
	...
	getValues(reference: InstanceBuffer) {
		return reference.getSelection(reference.getIndex(this));
	}

	get(
		data: "source",
		reference: InstanceBuffer
	): [x: number, y: number, w: number, h: number];
	get(
		data: "destination",
		reference: InstanceBuffer
	...
	get(data: any, reference: InstanceBuffer): any {
